...
export class DiscordRollRenderer {
  ...
  private doRender(roll: RollBase, root = false) {
    ...
    switch (type) {
      ...
      case 'mathfunction':
        ...
        break;
      case 'roll':
        return DiscordRollRenderer.renderRoll(roll as DieRoll);
      case 'fateroll':
        return DiscordRollRenderer.renderFateRoll(roll as FateDieRoll);
      case 'number': {
    ...
    if (root) {
      return DiscordRollRenderer.stripBrackets(render);
    }
  ...
  private renderGroup(group: GroupRoll) {
    ...
    const reply = DiscordRollRenderer.stripBrackets(replies[0]);
  ...
  private renderFunction(roll: MathFunctionRoll) {
    ...
    return `(${roll.op}${DiscordRollRenderer.addBrackets(render)} = ${roll.value})`;
  }

  private static addBrackets(str: string) {
    let render = str;
  ...
  private static stripBrackets(str: string) {
    let render = str;
